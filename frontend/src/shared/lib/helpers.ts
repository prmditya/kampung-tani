import type { DeviceStatusType, SensorType } from "./constants";
import { SENSOR_CONFIG, THEME_COLORS } from "./constants";
...
export function getSensorCardTheme(sensorType: SensorType) {
	return (
		THEME_COLORS.SENSOR_CARDS[sensorType] || THEME_COLORS.SENSOR_CARDS.moisture
	);
}
...
export const formatDateTime = (timestamp: string, timezone: string): string => {
	try {
		...
		const dateTransformed = date.toLocaleString("id-ID", {
			year: "numeric",
			month: "2-digit",
			day: "2-digit",
			hour: "2-digit",
