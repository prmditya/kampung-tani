		...
		async (
			...
			try {
				...
				const result = await fetchApi<{
					...
					size: number;
					pages: number;
				}>(`/sensors/?page=${pageNum}&size=${limitNum}`, abortSignal);
				if (!abortSignal?.aborted) {
		...
		async (
			...
			try {
				...
				const result = await fetchApi<{
					...
					size: number;
					pages: number;
				}>(`/devices?page=${page}&size=${limit}`, abortSignal);
...
export const useDeviceStats = (
	autoRefresh = false,
	interval = API_CONFIG.REFRESH_INTERVALS.DEVICE_STATUS,
): UseApiResult<DeviceStats> => {
	return useApiData<DeviceStats>("/devices/stats", autoRefresh, interval);
};

// Device status history
export const useDeviceStatusHistory = (
	deviceId: number | null,
): UseApiResult<DeviceStatusHistory[]> => {
	const endpoint = deviceId ? `/devices/${deviceId}/status-history` : "";
	const { data, loading, error, refetch } =
		useApiData<DeviceStatusHistoryResponse>(endpoint, false, 0);
...
export const useDevicesWithUptime = (
	initialPage = 1,
	initialLimit = 50,
): UsePaginatedApiResult<Device> & { devicesUptime: Record<number, any> } => {
			...
			for (const device of devices) {
				try {
					const response = await fetch(
						`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000/api/v3"}/devices/${device.id}/status-history?limit=1`,
						{
							headers: getAuthHeaders(),
						},
					);
	...
	const refetchWithUptime = useCallback(
		async (page?: number, limit?: number) => {
