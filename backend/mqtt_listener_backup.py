...
def on_message(client, userdata, msg):
    ...
    try:
        ...
        # Process legacy sensor data (both sensors/ and sensor/ patterns)
        elif (topic.startswith('sensors/') or topic.startswith('sensor/')) and topic.endswith('/data'):
    ...
    try:
        with get_db_cursor() as cursor:
            ...
            for reading in readings:
                ...
                if (sensor_type.startswith('#SYS_') or 
                    'Calibration' in sensor_type or 
                    'Coefficient' in sensor_type or
                    'Device_Address' in sensor_type or
                    'Device_Baud_Rate' in sensor_type):
                    continue
                ...
                try:
                    ...
                    cursor.execute(
                        ...
                        VALUES (%s, %s, %s, %s, %s)
                        """,
                        (device_id, sensor_type.lower(), value, unit, timestamp or 'NOW()')
                    )
                    logger.info(f"Inserted sensor data: {sensor_type} = {value} {unit} (raw: {raw_value})")
                except Exception as e:
                    logger.error(f"Error inserting sensor data {sensor_type}: {e}")
                    continue
                    
            logger.info(f"Saved {len([r for r in readings if not (r['sensor_type'].startswith('#SYS_') or 'Calibration' in r['sensor_type'] or 'Coefficient' in r['sensor_type'])])} readings for device {device_name}")
...
def process_sensor_data(topic: str, data: Dict[str, Any]):
    ...
    try:
        ...
        device_identifier = topic_parts[1]  # sem225, device_id, etc.
        ...
        with get_db_cursor() as cursor:
            ...
            # If device_identifier is numeric, try as ID first
            if device_identifier.isdigit():
                cursor.execute("SELECT id FROM devices WHERE id = %s", (int(device_identifier),))
                device_row = cursor.fetchone()
                if device_row:
                    device_id = device_row[0]
            ...
            if device_id is None:
                cursor.execute("SELECT id FROM devices WHERE name = %s", (device_identifier,))
                device_row = cursor.fetchone()
                if device_row:
                    device_id = device_row[0]
                else:
                    ...
                    cursor.execute(
                        ...
                        RETURNING id
                        """,
                        (device_identifier, 'soil_sensor', 'online', 'field')
                    )
                    device_id = cursor.fetchone()[0]
                    logger.info(f"Created new device: {device_identifier} with ID {device_id}")
            ...
            logger.info(f"Saved sensor data for device {device_identifier} (ID: {device_id}): {data['sensor_type']} = {data['value']}")
